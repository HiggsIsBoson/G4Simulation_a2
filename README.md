...
- 粒子の生成 (`src/PrimaryGeneratorAction.cc`) : 原点で2γもしくは3γを生成 ("イベント")。
  ...
  * 3γ : Ore-Powel分布 ([この論文](https://arxiv.org/pdf/hep-ph/0311002)の式16) に従ったエネルギーの組み合わせで, エネルギー・運動量保存を満たしながら等方的に生成。
...
## Gitの使い方  
Gitはコード管理ツールで、セーブしたり過去のバージョンに遡ったり、コードの開発を大人数でやるときの更新とかに便利です。
機能は無限にあるのできりがないですが、ここでは自分で編集を加えたものを新しい「ブランチ」として定義し、レポジトリにアップロードする方法を書きます。
- 自分のbranchを作成 : `git checkout -b <branch_name>`
- ファイルの変更・追加を反映 : `git add <files>` (大元のディレクトリで`git add .`でも可)
    * ファイルをrename/removeするときは`git mv`とか`git rm`を使う必要がある。さもなくばローカルでは消えてるけど、リモートではファイルが残り続ける（めんどい仕様ですね）
    * 捕捉されている変更の確認 : `git status`
- 変更をcommit : `git commit -m "適当なコメント"` (この「コミット」という操作がセーブの基本単位となる)
- リポジトリにpush : `git push origin <branch_name>`
