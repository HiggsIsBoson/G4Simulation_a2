...
- Run 単位の処理の設定 (`src/RunAction.cc`) : Eventを束ねたものがRunである。ここではファイルに書き出す変数などの定義を行っている。Modeに応じて出力されるTTreeの構造が異なる。
...
- 検出器の構成を変更したくなったらここをいじればよい。ソースコード（`.cc`ファイル）を変更した後はコンパイルをやり直す (i.e. `build/`に行って`make -j 8`を実行)。新しいソースファイルを追加したときは`cmake`からやり直す。
